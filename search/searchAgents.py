    ...
    def getSuccessors(self, state: Any):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            if not self.walls[nextx][nexty]:
                ...
                new_visited = tuple(v or (nextposition == corner)
                                    for v, corner in zip(visited, self.corners))

                successors.append(((nextposition, new_visited), action, 1))
...
def cornersHeuristic(state: Any, problem: CornersProblem):
    ...
    def mst_cost(nodes):
        ...
        while remaining:
            min_edge = min(manhatten(a, b) for a in in_tree for b in remaining)
            for b in remaining:
                if any(manhatten(a, b) == min_edge for a in in_tree):
                    in_tree.add(b)
                    remaining.remove(b)
                    total += min_edge
                    break
        return total
    dist_to_nearest = min(manhatten(position, corner) for corner in unvisited)

    tree_cost = mst_cost(unvisited)

    return dist_to_nearest + tree_cost
...
def foodHeuristic(state: Tuple[Tuple, List[List]], problem: FoodSearchProblem):
    ...
    mstWeight = 0
    if len(foodList) > 1:
        mstWeight = calculateMSTWeight(foodList, problem)

    return distToClosestFood + mstWeight


def calculateMSTWeight(nodes, problem):
    n = len(nodes)
    if n <= 1:
        return 0
    ...
    for _ in range(n):
        # Vind node met kleinste edge
        u = -1
        for i in range(n):
            if not visited[i] and (u == -1 or min_edges[i] < min_edges[u]):
                u = i

        visited[u] = True
        total_weight += min_edges[u]

        # Update afstanden naar buren
        for v in range(n):
            if not visited[v]:
                d = mazeDistance(nodes[u], nodes[v], problem.startingGameState)
                if d < min_edges[v]:
                    min_edges[v] = d
